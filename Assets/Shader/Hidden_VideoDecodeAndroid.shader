...
Shader "Hidden/VideoDecodeAndroid"
{
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: RGBAExternal_To_RGBA
    {
      ...
      {
          float3 objSpaceCameraPos = mul(unity_WorldToObject, float4(_WorldSpaceCameraPos.xyz, 10)).xyz;
      ...
      float4 AdjustForColorSpace(float4 color)
      {
          ...
          return float4((sRGB * ((sRGB * ((sRGB * 305306011) + 682171111)) + 12522878)), color.a);
          #endif
      }
    ...
    Pass // ind: 2, name: RGBASplitExternal_To_RGBA
    {
      ...
      OUT_Data_Vert vert(appdata_t in_v)
      {
          OUT_Data_Vert out_v;
          out_v.vertex = UnityObjectToClipPos(in_v.vertex);
          out_v.textureCoordSplit.xz = (float2(((5 * in_v.texcoord.x) * _MainTex_ST.x), (in_v.texcoord.y * _MainTex_ST.y)) + _MainTex_ST.zw);
          out_v.textureCoordSplit.y = (out_v.textureCoordSplit.x + (5 * _MainTex_ST.x));
          return out_v;
      }
